...
public class JwtTokenUtil {
    ...
    //@Value("${jwt.secret}")
    private String secret = "diy-secret"; // JWT 加解密使用的密钥
    //@Value("${jwt.expiration}")
    private Long expiration = 604800L; // JWT 的超期限时间 （ 60*60*24）24小时失效
    ...
    public String generateToken(UserEntity user) {
        ...
        claims.put(CLAIM_KEY_USERNAME, user.getMobile()); // 从 security 框架 UserDetails 中获取用户名
    ...
    public boolean validateToken(String token, UserEntity user) {
        ...
        return username.equals(user.getMobile()) && !isTokenExpired(token); // 本类内新建方法
    }
